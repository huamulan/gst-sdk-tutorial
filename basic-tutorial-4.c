...
int main(int argc, char *argv[]) {
    ...
    do {
        ...
        /* Parse message */
        if (msg != NULL) {
            g_print("Got message in 100 milliseconds, handle_message\n");
            handle_message (&data, msg);

        } else {
            g_print("We got no message in 100 milliseconds, this means the timeout expired\n");
...
static void handle_message (CustomData *data, GstMessage *msg) {
    ...
    switch (GST_MESSAGE_TYPE (msg)) {
        case GST_MESSAGE_ERROR:
            g_print ("GST_MESSAGE_ERROR reached.\n");
        ...
        case GST_MESSAGE_DURATION:
            g_print ("GST_MESSAGE_DURATION reached.\n");
            /* The duration has changed, mark the current one as invalid */
            data->duration = GST_CLOCK_TIME_NONE;
            break;

        case GST_MESSAGE_STATE_CHANGED: {
                                            g_print ("GST_MESSAGE_STAGE_CHANGED reached.\n");
                                            ...
                                            if (GST_MESSAGE_SRC (msg) == GST_OBJECT (data->playbin2)) {
                                                ...
                                                if (data->playing) {
                                                    ...
                                                    if (gst_element_query (data->playbin2, query)) {
                                                        //seek from start to end.
                                                        gst_query_parse_seeking (query, NULL, &data->seek_enabled, &start, &end);
                                                        if (data->seek_enabled) {
                                                            g_print ("Seeking is ENABLED from %" GST_TIME_FORMAT " to %" GST_TIME_FORMAT "\n",
                                                                    GST_TIME_ARGS (start), GST_TIME_ARGS (end));
                                                        } else {
                                                            g_printerr ("Seeking is DISABLED for this stream.\n");
                                                        }
                                                    } else {
