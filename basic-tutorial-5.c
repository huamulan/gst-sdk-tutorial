...
static void realize_cb (GtkWidget *widget, CustomData *data) {
    ...
    g_print("realize_cb, this function is called when GUI toolkit creates the physical window\n");
    ...
    g_print("GDK_WINDOWING_WIN32");
    window_handle = (guintptr)GDK_WINDOW_HWND (window);

#elif defined (GDK_WINDOWING_QUARTZ)
    g_print("GDK_WINDOWING_QUARTZ");
    window_handle = gdk_quartz_window_get_nsview (window);

#elif defined (GDK_WINDOWING_X11)
    g_print("GDK_WINDOWING_X11");
...
static gboolean expose_cb (GtkWidget *widget, GdkEventExpose *event, CustomData *data) {
    if (data->state < GST_STATE_PAUSED) {
        g_print("----->need use cairo to re-draw video window.\n");
...
static void create_ui (CustomData *data) {
    ...
    g_print("----->create_ui and set callbacks\n");
    ...
    //Very similar with QT's signal/slot mechanism.
